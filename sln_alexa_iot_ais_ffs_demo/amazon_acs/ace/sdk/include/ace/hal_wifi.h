...
/**
 * @brief The maximum number of WEP keys for a profile
 */
#define aceWifiHal_MAX_WEP_KEY_NUM     4
...
typedef struct {
    ...
    aceWifiHal_wepKey_t wepKeys[aceWifiHal_MAX_WEP_KEY_NUM];
...
/**
 * @brief Wi-Fi hal reset modes.
 *
 * The underlying Wi-Fi hal reset modes. Allow control over soft reset (no
 * Wi-Fi hardware) or hard reset (entire Wi-Fi hardware reset).
 */
typedef enum
{
    aceWifiHal_SOFT_RESET = 0, /**< soft reset mode, does not reset Wi-Fi HW */
    aceWifiHal_HARD_RESET,     /**< hard reset, including Wi-Fi chip reset */
} aceWifiHal_resetMode_t;

/**
 * @brief Wi-Fi hal restart request
 */
typedef struct {
    aceWifiHal_resetMode_t reset_mode; /**< soft/hard reset mode */
} aceWifiHal_restartConfig_t;
...
/**
 * @brief Restart WiFi HAL
 *
 * @param restartConfig: a pointer to @ref aceWifiHal_restartConfig_t config,
 *  which has member @ref aceWifiHal_resetMode_t to request a soft restart or
 *  hard restart.
 *  Hard reset causes underlying Wi-Fi SOC/hardware to get reset as well.
 *  For SoCs that support multiple radio MACs such as Bluetooth and Wi-Fi
 *  a hard reset, resets all (Bluetooth, Wi-Fi) radio MACs.
 *
 * @return Return ACE_STATUS_OK for success, error codes for failure.
 */
ace_status_t aceWifiHal_restart(aceWifiHal_restartConfig_t* restartConfig);
