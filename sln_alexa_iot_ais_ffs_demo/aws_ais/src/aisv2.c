...
#define AISV2_TASK_PRIORITY   tskIDLE_PRIORITY + 3
...
__attribute__((
    section(".ocram_non_cacheable_bss"))) static ais_seq_directive_slot_t s_directivePool[AIS_SEQUENCE_BUFFER_SLOTS];
__attribute__((
    section(".ocram_non_cacheable_bss"))) static ais_seq_speaker_slot_t s_speakerPool[AIS_SEQUENCE_BUFFER_SLOTS];
...
static void AIS_StateMicrophone(ais_handle_t *handle)
{
    ...
    char long_buf1[LONG_STR_BUFSIZE] = {0};
    char long_buf2[LONG_STR_BUFSIZE] = {0};
    ...
    if (kStatus_Success == AIS_AppCallback_Microphone(&micData, &size))
    {
        ...
        if (kStatus_Success == ret)
        {
            ...
            {
                sln_long_to_str(offsetBackup, long_buf1);
                sln_long_to_str(stream->audio.audioData.offset, long_buf2);
                configPRINTF(("WARNING: mic offset updated by other task, old offset: %s, new offset: %s\r\n",
                              long_buf1, long_buf2));
        ...
        if (s_publishMicErrCount >= AIS_MIC_PUBLISH_ERR_MAX)
        {
            ...
            {
                configPRINTF(("Publish mic err threshold reached, triggering a reconnection\r\n"));
            }
        }
    }
}
...
void AIS_MessageResequence(ais_handle_t *handle, aisTopic_t topic, ais_process_func_t processFunc)
{
    ...
    /* Run all verifications only if there is something in the re-sequencing list */
    if (seqBufSize)
    {
        bool processed = false;
        /*
         * In the case of speaker buffer overflow, it's possible that queue stores some out dated
         * packages
         */
        ...
        /* Check to see if there are any sequences for topic in the re-sequencing buffer. */
        while ((seqBufSize > 0) && (list[0].seq == handle->topicSequence[topic]))
        {
            configPRINTF(("Trying to reprocess on topic: %d, seq: %d\r\n", topic, list[0].seq));
            ...
            if (!processFunc(handle, list[0].data, list[0].size))
            {
                if (topic == AIS_TOPIC_SPEAKER)
                {
                    ...
                    {
                        configPRINTF(("Skip sending another overrun, expecting the previous one to be processed\r\n"));
                    }
                }

                configPRINTF(("Failed to reprocess on topic: %d, seq: %d\r\n", topic, list[0].seq));
                break;
            }

            configPRINTF(("Successfully reprocessed on topic: %d, seq: %d\r\n", topic, list[0].seq));
        ...
        /* If out of sequence message was processed, either stop the timer
         * (if no other out of sequence messages left), or update the time-stamp.
         * This should be done no matter what the topic is. */
        if (processed)
        {
            if ((seqBufSize == 0) && (*timer != 0))
            {
                /* Stop timer when no re-sequencing needed */
                *timer = 0;
            }
            else if ((seqBufSize > 0) && (*timer == 0))
            {
                /* Restart timer when out of sequence */
                *timer = xTaskGetTickCount();
            }
        }

        /* Check to see if timer has expired before receiving message. */
        if ((false == processed) && (*timer != 0))
        {
            if (AIS_TimeoutCheck(*timer, AIS_MSG_SEQ_TIMEOUT_MSEC))
            {
                configPRINTF(("Reached sequence timeout of %d sec on topic %d, last: %d, now: %d\r\n",
                              AIS_MSG_SEQ_TIMEOUT_MSEC / 1000, topic, *timer, xTaskGetTickCount()));
                *timer = 0;
...
/*! @brief Internal check to see if the secrets have been rotated for all topics. */
static status_t AIS_CheckSecretRotationComplete(ais_handle_t *handle)
{
...
static void AIS_StateBackground(ais_handle_t *handle)
{
    ...
    /* Check for a rotating secret. */
    if (kStatus_Success == AIS_CheckSecretRotationComplete(handle))
    {
...
status_t AIA_SmartHomeRegistration(ais_handle_t *handle)
{
    ...
    ret = AIA_AlexaSmartHomeAddOrUpdateReport(handle);
...
status_t AIS_Connect(ais_handle_t *handle)
{
    ...
    /* Send Smart Home Registration */
    AIA_SmartHomeRegistration(handle);
    ...
    /* Mark the static buffers as free. */
    _initPool();
...
{
    return ((xTaskGetTickCount() - savedTick) >= (timeout / portTICK_PERIOD_MS));
}
