...
#define AIS_CAPABILITY_VERSION_MAX_SIZE (4)
#define AIS_CAPABILITY_VERSION          "1.1"
#define AIS_CAPABILITY_SMART_VERSION    "1.0"
...
cJSON *JSON_BuildHeader(ais_handle_t *handle, const char *name, const char **id)
{
    ...
    AIA_build_uuid(handle, messageId);
...
const char *JSON_BuildEvent(ais_handle_t *handle, const char *name, ais_json_t *jsonObj, bool addPayload)
{
    ...
    if (addPayload)
    {
        ...
        {
            jsonObj->payload = cJSON_CreateObject();
        }

        cJSON_AddItemToObject(event, "payload", jsonObj->payload);
    }
    else
    {
...
cJSON *JSON_BuildCapability(const char *name, const char *version)
{
    ...
    cJSON_AddItemToObject(cap, "version", cJSON_CreateString(version));
...
{
    cJSON *cap = JSON_BuildCapability("Speaker", AIS_CAPABILITY_VERSION);
...
{
    cJSON *cap = JSON_BuildCapability("Microphone", AIS_CAPABILITY_VERSION);
...
{
    cJSON *cap = JSON_BuildCapability("Alerts", AIS_CAPABILITY_VERSION);
...
{
    return JSON_BuildCapability("EndpointForwarding", AIS_CAPABILITY_SMART_VERSION);
}

cJSON *JSON_BuildCapabilityClock(void)
{
    return JSON_BuildCapability("Clock", AIS_CAPABILITY_VERSION);
}

cJSON *JSON_BuildCapabilitySystem(ais_handle_t *handle)
{
    cJSON *cap = JSON_BuildCapability("System", AIS_CAPABILITY_VERSION);

    cJSON *config = cJSON_CreateObject();
    cJSON_AddItemToObject(cap, "configurations", config);
    cJSON_AddItemToObject(config, "firmwareVersion", cJSON_CreateString(handle->config->firmwareVersion));
    if (0 == strncmp(AIS_CAPABILITY_VERSION, "1.0", AIS_CAPABILITY_VERSION_MAX_SIZE))
    {
        cJSON_AddItemToObject(config, "locale", cJSON_CreateString(handle->config->locale));
    }
    else if (0 == strncmp(AIS_CAPABILITY_VERSION, "1.1", AIS_CAPABILITY_VERSION_MAX_SIZE))
    {
        uint32_t i                      = 0;
        cJSON *localesArray             = cJSON_CreateArray();
        cJSON *localesCombinationsArray = cJSON_CreateArray();

        for (i = 0; i < handle->config->numberOfLocales; i++)
        {
            cJSON_AddItemToArray(localesArray, cJSON_CreateString(handle->config->localesSupported[i]));
        }

        for (i = 0; i < handle->config->numberOfLocaleCombinations; i++)
        {
            cJSON_AddItemToArray(localesCombinationsArray,
                                 cJSON_CreateString(handle->config->localeCombinationsSupported[i]));
        }

        cJSON_AddItemToObject(config, "locales", localesArray);
        cJSON_AddItemToObject(config, "localeCombinations", localesCombinationsArray);
    }
