...
#define AIA_TIME_EPOCH_ADJUST (2208988800ULL)
...
#define AIS_SEQUENCE_BUFFER_SLOTS 8
...
#define AIS_MSG_OPEN_SPEAKER_TIMEOUT_MSEC (10000)
...
#define AIS_EVENT_ENDPOINT_FORWARDING "EndpointForwarding"
#define AIS_EVENT_LOCALES_REPORT "LocalesReport"
#define AIS_EVENT_LOCALES_CHANGED "LocalesChanged"

/*! @brief AVS Smart Home Event Values */
#define AIA_EVENT_PAYLOAD_SCOPE_TOKEN_TYPE "BearerType" // https://oauth.net/2/bearer-tokens/
...
/*! @brief AIA Smart Home Definitions */
#define AIA_SMART_HOME_ALEXA_PAYLOAD_VERSION    "3"

#define AIA_SMART_HOME_NAMESPACE_ALEXA    "Alexa"
#define AIA_SMART_HOME_NAMESPACE_ALEXA_MODE_CONTROLLER    "Alexa.ModeController"
#define AIA_SMART_HOME_NAMESPACE_ALEXA_TOGGLE_CONTROLLER    "Alexa.ToggleController"
#define AIA_SMART_HOME_NAMESPACE_ALEXA_POWER_CONTROLLER    "Alexa.PowerController"
#define AIA_SMART_HOME_NAMESPACE_ALEXA_RANGE_CONTROLLER    "Alexa.RangeController"
#define AIA_SMART_HOME_NAMESPACE_ALEXA_BRIGHTNESS_CONTROLLER    "Alexa.BrightnessController"

#define AIA_SMART_HOME_UNCERTAINTY_IN_MS (1)
#define AIA_SMART_HOME_TIME_BUFFER_SIZE (21)
...
/*! @brief Values for Power Controller Message Types */
typedef enum
{
    AIS_SMART_HOME_POWER_CONTROLLER_TURN_ON = 0,
    AIS_SMART_HOME_POWER_CONTROLLER_TURN_OFF,
} ais_smart_home_power_controller_type_t;

/*! @brief Values for Toggle Controller Message Types */

typedef enum
{
    AIS_SMART_HOME_TOGGLE_CONTROLLER_TURN_ON = 0,
    AIS_SMART_HOME_TOGGLE_CONTROLLER_TURN_OFF,
} ais_smart_home_toggle_controller_type_t;

/*! @brief Values for Range Controller Message Types */

typedef enum
{
    AIS_SMART_HOME_RANGE_CONTROLLER_SET_RANGE = 0,
    AIS_SMART_HOME_RANGE_CONTROLLER_ADJUST_RANGE,
} ais_smart_home_range_controller_type_t;

/*! @brief Values for Mode Controller Message Types */

typedef enum
{
    AIS_SMART_HOME_MODE_CONTROLLER_SET = 0,
    AIS_SMART_HOME_MODE_CONTROLLER_ADJUST,
} ais_smart_home_mode_controller_type_t;

/*! @brief Values for Brightness Controller Message Types */
typedef enum
{
    AIS_SMART_HOME_BRIGHTNESS_CONTROLLER_SET = 0,
    AIS_SMART_HOME_BRIGHTNESS_CONTROLLER_ADJUST,
} ais_smart_home_brightness_controller_type_t;


/*! @brief Values for Different Controller Types */
typedef enum
{
    AIS_SMART_HOME_POWER_CONTROLLER,
    AIS_SMART_HOME_TOGGLE_CONTROLLER,
    AIS_SMART_HOME_RANGE_CONTROLLER,
    AIS_SMART_HOME_MODE_CONTROLLER,
    AIS_SMART_HOME_BRIGHTNESS_CONTROLLER
} ais_smart_home_controller_type_t;
...
/*! @brief AVS Smart Home Power Controller Endpoint */
typedef struct
{
    bool on; /* The value whether the power is on or off */
} ais_avs_power_controller_t;

/*! @brief AVS Smart Home Brightness Controller Endpoint */
typedef struct
{
    uint32_t brightness; /* Brightness as a percentage */
    uint32_t brightnessDelta; /* Brightness as a percentage */
} ais_avs_brightness_controller_t;

/*! @brief AVS Smart Home Mode Controller Endpoint */
typedef struct
{
    uint32_t mode; /* Mode as an object number */
    uint32_t modeDelta; /* The number of modes to cycle through */
} ais_avs_mode_controller_t;

/*! @brief AVS Smart Home Range Controller Endpoint */
typedef struct
{
    uint32_t range;             /* range value */
    uint32_t rangeDelta;        /* Amount in which to adjust */
    uint32_t rangeDeltaDefault; /* Whether the user made an explicit value change */
} ais_avs_range_controller_t;

/*! @brief AVS Smart Home Range Controller Endpoint */
typedef struct
{
    bool on; /* Toggle off or on */
} ais_avs_toggle_controller_t;

typedef struct
{
    char *endpointId;
    ais_avs_brightness_controller_t *brightnessController;
    ais_avs_power_controller_t *powerController;
    ais_avs_range_controller_t *rangeController;
    ais_avs_toggle_controller_t *toggleController;
    ais_avs_mode_controller_t *modeController;
} ais_avs_smart_home_endpoint_t;

typedef struct {
    uint32_t number_of_endpoints;
    QueueHandle_t p_smartHomeQueue;
    SemaphoreHandle_t s_smartHomeLock;
    ais_avs_smart_home_endpoint_t *smart_home_endpoint;
} ais_avs_smart_home_t;
...
typedef struct _ais_config_t
{
    ...
    /* Locale capabilities */
    uint32_t numberOfLocales;
    uint32_t numberOfLocaleCombinations;

    /* System capabilities */
    char firmwareVersion[16];
    char locale[12];
    char **localesSupported;
    char **localeCombinationsSupported;
...
/*! @brief AIA Smart Home Response Payload */
typedef struct _aia_smart_home_response_payload
{
    char *correlationToken;
    char *endpointId;
    ais_json_t context; /* https://developer.amazon.com/en-US/docs/alexa/alexa-voice-service/reportable-state-properties.html#property-object */
    ais_json_t payload; /* https://developer.amazon.com/en-US/docs/alexa/alexa-voice-service/alexa.html#changereport */
} aia_smart_home_response_payload_t;

/*! @brief AIA Smart Home Response Payload */
typedef struct _aia_smart_home_change_report_payload
{
    char *correlationToken;
    char *endpointId;
    ais_json_t context; /* https://developer.amazon.com/en-US/docs/alexa/alexa-voice-service/reportable-state-properties.html#property-object */
    ais_json_t payload; /* https://developer.amazon.com/en-US/docs/alexa/alexa-voice-service/alexa.html#changereport */
} aia_smart_home_change_report_payload_t;

/*! @brief Payload for task communication via queue. */
typedef struct
{
    ais_smart_home_controller_type_t controller_type; /* Type as defined in ais_smart_home_controller_type */
    uint32_t controller_directive_type; /* Controller type for example ais_smart_home_power_controller_type */
    aia_smart_home_response_payload_t *response; /* Parameters for the response that needs to be sent back */
    ais_avs_smart_home_endpoint_t *endpoint; /* Pointer to the endpoint the message is associated with */
} aia_smart_home_queue_payload_t;
...
typedef struct _ais_handle_t
{
    ...
    ais_avs_smart_home_t smart_home;
...
/*!
 * @brief Sends the AddOrUpdateReport to AVS for Smart Home Registration
 *
 * This function constructs the Report in JSON and sends it to the service for
 * Smart Home registration
 *
 * @param handle Pointer to AIA interface handle
 * @return kStatus_Success on success, other on failure
 */
status_t AIA_AlexaSmartHomeAddOrUpdateReport(ais_handle_t *handle);

/*!
 * @brief Connect to the Smart Home Service
 *
 * This function sends an AddOrUpdateReport to the AIA service to register
 * the Smart Home Endpoint
 *
 * @param handle Pointer to AIA interface handle
 * @return kStatus_Success on success, other on failure
 */
status_t AIA_SmartHomeRegistration(ais_handle_t *handle);
/*!
...
/*!
 * @brief Function to send LocalesChanged event when an on device or external
 *        Third Party app changes the Wake Word Model Locale
 *
 * @param handle Pointer to AIS interface handle
 * @param locale The locale changed for the Amazon Wake Word model
 *
 * @return kStatus_Success on success, other on failure
 */
status_t AIS_EventLocalesChanged(ais_handle_t *handle, char *locale);

/*!
 * @brief Function to send LocalesReport event in response to a SetLocale directive
 *
 * @param handle Pointer to AIS interface handle
 * @param locale The locale changed for the Amazon Wake Word model
 *
 * @return kStatus_Success on success, other on failure
 */
status_t AIS_EventLocalesReport(ais_handle_t *handle, char *locale);

/*!
 * @brief Function to send Endpoint Forwarding messages to AIA
 *
 * @param handle Pointer to AIS interface handle
 * @param jsonEvent Contains the JSON message to be wrapped and sent.
 *
 * @return kStatus_Success on success, other on failure
 */
status_t AIS_EventSmartHomeEndpointForwarding(ais_handle_t *handle, ais_json_t *jsonEvent);
...
/*!
 * @brief This Function retrieves the generated Smart Home Endpoint descriptor JSON
 *
 * When Smart Home Directives are enabled, this function retrieves the generated JSON string
 * after using the Smart Home Endpoint conversion tool.
 *
 * @param *handle Reference to current ais_handle_t in use
 *
 * @return The retrieved JSON object or NULL if doesn't exist
 */
cJSON *AIA_DiscoveryEndpointGetAddOrUpdateReportJson(ais_handle_t *handle);

/*!
 * @brief This Function retrieves the endpoint structure with the controllers enabled
 *
 * When a message comes from the smart home service, it will have an endpoint, use this function
 * to get the structure related to the endpoint.
 *
 * @param *handle Reference to current ais_handle_t in use
 * @param *endpointId String of the endpointID
 *
 * @return Returns a pointer to the endpoint structure or NULL if not found
 */
ais_avs_smart_home_endpoint_t *AIA_SmartHomeGetEndpoint(ais_handle_t *handle, char *endpointId);

/*!
 * @brief This Function Sends the response for the endpoint when requested or a change occurs
 *
 * When Smart Home Directives are enabled, this function constructs the response payload and sends
 *
 * @param *handle Reference to current ais_handle_t in use
 * @param *response message to indicate the new value
 *
...
/*!
 * @brief Brightness Controller callback for the application to execute the power controller command
 *
 * When the AIA interface gets a toggle command, the task will call this callback for the developer to action on it.
 *
 * @param *handle Reference to current ais_handle_t in use
 * @param *smart_home_msg payload for the smart home message
 *
 * @return Success or failure
 */
...
/*!
 * @brief Mode Controller callback for the application to execute the power controller command
 *
 * When the AIA interface gets a toggle command, the task will call this callback for the developer to action on it.
 *
 * @param *handle Reference to current ais_handle_t in use
...
/*!
 * @brief Range Controller callback for the application to execute the power controller command
 *
 * When the AIA interface gets a toggle command, the task will call this callback for the developer to action on it.
 *
 * @param *handle Reference to current ais_handle_t in use
...
/*!
 * @brief Toggle Controller callback for the application to execute the power controller command
 *
 * When the AIA interface gets a toggle command, the task will call this callback for the developer to action on it.
 *
 * @param *handle Reference to current ais_handle_t in use
...
/*!
 * @brief Power Controller callback for the application to execute the power controller command
 *
 * When the AIA interface gets a toggle command, the task will call this callback for the developer to action on it.
 *
 * @param *handle Reference to current ais_handle_t in use
...
/*!
 * @brief Callback for the SetLocale which changes the region on the device
 *
 * This callback will allow the relative application functions associated with a region change
 *
 * @param *handle Reference to current ais_handle_t in use
 * @param *locale String representation of the new region
 *
 * @return Success or failure
 */
...
/*!
 * @brief Generates a random ascii uuid for unique message ID creation
 *
 * AIA and AVS require a Unique Identifier for messages, this function is to create one
 *
 * @param *handle Reference to current ais_handle_t in use
 * @param *out output buffer for unique ID
 *
 * @return Success or failure
 */
void AIA_build_uuid(ais_handle_t *handle, uint8_t *out);

/*!
 * @brief Maps ais_buffer_state_t to a predefined string describing the state
 *
 * @param state An ais_buffer_state_t variable describing the buffer state
 *
 * @return Pointer to the predefined string describing the state
 */
const char *AIS_MapBufferState(ais_buffer_state_t state);

/*!
 * @brief Check if a given timeout expired, based on a previously saved tick
 *
 * @param savedTick Saved system tick based on which the timeout is checked
 * @param timeout   Value of the timeout to be checked, in milliseconds
 *
 * @return true if the timeout expired, false otherwise
 */
bool AIS_TimeoutCheck(TickType_t savedTick, uint32_t timeout);
...
/*!
 * @brief Sends the JSON payload to the Handler to process
 *
 * @param handle Pointer to AIS interface handle that will be initialized
 *
 * @return Success or failure
 ...
 *
 * @param handle Pointer to AIS interface handle that will be initialized
 * @param payload Incoming message from the service to process
 *
 * @return Success or failure
 */
...
/*!
 * @brief Sends the JSON payload to the Power Controller Handler to process
 *
 * @param handle Pointer to AIS interface handle that will be initialized
 * @param message Incoming message from the service to process
 *
 * @return Success or failure
 */
status_t AIA_AlexaSmartHomePowerControllerHandler(ais_handle_t *handle, cJSON *message);

/*!
 * @brief Sends the JSON payload to the Mode Controller Handler to process
 *
 * @param handle Pointer to AIS interface handle that will be initialized
 * @param message Incoming message from the service to process
 *
 * @return Success or failure
 */
status_t AIA_AlexaSmartHomeModeControllerHandler(ais_handle_t *handle, cJSON *message);

/*!
 * @brief Sends the JSON payload to the Range Controller Handler to process
 *
 * @param handle Pointer to AIS interface handle that will be initialized
 * @param message Incoming message from the service to process
 *
 * @return Success or failure
 */
status_t AIA_AlexaSmartHomeRangeControllerHandler(ais_handle_t *handle, cJSON *message);

/*!
 * @brief Sends the JSON payload to the Toggle Controller Handler to process
 *
 * @param handle Pointer to AIS interface handle that will be initialized
 * @param message Incoming message from the service to process
 *
...
/*!
 * @brief Sends the JSON payload to the Brightness Controller Handler to process
 *
 * @param handle Pointer to AIS interface handle that will be initialized
 * @param json Incoming message from the service to process
 *
 * @return Success or failure
 */
...
/*!
 * @brief Creates the context entry for Brightness Controller
 *
 * @param handle Pointer to AIS interface handle that will be initialized
 * @param endpoint Pointer to the endpoint for the controller to reference
 *
 * @return true if the timeout expired, false otherwise
 */
...
/*!
 * @brief Creates the context entry for Mode Controller
 *
 * @param handle Pointer to AIS interface handle that will be initialized
 * @param endpoint Pointer to the endpoint for the controller to reference
 *
 * @return true if the timeout expired, false otherwise
 */
cJSON *AIA_AlexaSmartHomeModeControllerCreateResponseContext(ais_handle_t *handle,
                                                             ais_avs_smart_home_endpoint_t *endpoint);

/*!
 * @brief Creates the context entry for Power Controller
 *
 * @param handle Pointer to AIS interface handle that will be initialized
 * @param endpoint Pointer to the endpoint for the controller to reference
 *
 * @return true if the timeout expired, false otherwise
 */
cJSON *AIA_AlexaSmartHomePowerControllerCreateResponseContext(ais_handle_t *handle,
                                                              ais_avs_smart_home_endpoint_t *endpoint);

/*!
 * @brief Creates the context entry for Range Controller
 *
 * @param handle Pointer to AIS interface handle that will be initialized
 * @param endpoint Pointer to the endpoint for the controller to reference
 *
 * @return true if the timeout expired, false otherwise
 */
cJSON *AIA_AlexaSmartHomeRangeControllerCreateResponseContext(ais_handle_t *handle,
                                                              ais_avs_smart_home_endpoint_t *endpoint);

/*!
 * @brief Creates the context entry for Toggle Controller
 *
 * @param handle Pointer to AIS interface handle that will be initialized
 * @param endpoint Pointer to the endpoint for the controller to reference
 *
