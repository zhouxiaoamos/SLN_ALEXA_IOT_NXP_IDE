...
#include "fsl_dcp_ext.h"

#include "sln_RT10xx_RGB_LED_driver.h"

/* Update using solution UART and TCP mechanism */
#if BOOTLOADER_FWUPDATE_ENABLE
...
/* Update firmware using Solution UART and TCP mechanism */
#if BOOTLOADER_FWUPDATE_ENABLE

static void CheckForFwUpdate(void)
{
    ...
    if (((SLN_FLASH_NO_ERROR == status) && (0 == fwupdateflag)) || (BUTTON_OTWPressed()))
    {
        ...
        if (SLN_FLASH_NO_ERROR == status)
        {
            ...
            if (METHOD_SLN_OTA == methodFlag)
            {
                /* We have an ota triggered */
#if BOOTLOADER_TCP_OTA_ENABLE
                otaAppStart();
#endif
            }
            else if ((METHOD_SLN_OTW == methodFlag) || (BUTTON_OTWPressed()))
            {
...
#if BOOTLOADER_FWUPDATE_ENABLE
        // Check for a valid pending update
        CheckForFwUpdate();
#endif
    }
