...
void Run_OTA_Task(void)
{
    ...
    if (xTaskCreate(runDemoTask, "AWS_OTA_Task", 2048, &mqttDemoContext, configMAX_PRIORITIES - 1, NULL) != pdPASS)
    {
        configPRINTF(("xTaskCreate AWS_OTA_Task failed\r\n"));
    }
}
                   ...
                   const IotNetworkInterface_t *pNetworkInterface)
{
    ...
    if (xTaskCreate(reconnectTask, "Reconnect_task", 1400, NULL, configMAX_PRIORITIES - 1, NULL) != pdPASS)
    {
        configPRINTF(("xTaskCreate Reconnect_task failed\r\n"));
        goto exit;
    }

    /* Create a task for getting signaled when OTA finishes and a reset is needed */
    if (xTaskCreate(otaDoneTask, "ota_done_task", 1024, NULL, tskIDLE_PRIORITY + 1, &otaDoneTaskHandle) != pdPASS)
    {
