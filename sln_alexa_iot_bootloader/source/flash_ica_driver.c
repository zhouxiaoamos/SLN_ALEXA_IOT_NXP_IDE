...
int32_t FICA_app_program_ext_init(int32_t newimgtype)
{
    ...
    s_newAppCurrLen = 0;
    FICA_clear_buf(s_appImgBuffer, 0xFF, SECTOR_SIZE);
...
__attribute__((section(".ramfunc.$SRAM_OC_NON_CACHEABLE"))) int32_t FICA_read_flash_id(uint8_t *pid)
{
    uint8_t id;
    ...
    // Read hyperflash ID
    ret = sln_flash_ops_get_flash_id(FLEXSPI, &id);
