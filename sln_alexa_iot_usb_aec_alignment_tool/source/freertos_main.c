...
#define sound_play_task_NAME     "SoundPlay"
#define sound_play_task_PRIORITY 4
#define sound_play_task_STACK    512

#define STREAMER_PACKET_SIZE PCM_AMP_DATA_SIZE_20_MS
#define STREAMER_VOLUME      100

#define DELAY_STARTUP             5000
#define DELAY_BETWEEN_SOUND_PLAYS 5000
...
TaskHandle_t xSoundPlayTaskHandle;
...
static uint32_t s_wakeWordsDetected       = 0;
...
void soundPlayTask(void *arg)
{
    uint8_t *sound           = (uint8_t *)med_system_alerts_melodic_01_mono_wav;
    uint32_t soundSize       = sizeof(med_system_alerts_melodic_01_mono_wav);
    uint32_t soundDataPlayed = 0;
    uint32_t packetSize      = 0;

    /* Assure that the other tasks are initialized */
    vTaskDelay(DELAY_STARTUP);
    ...
    while (1)
    {
        /* Skip the packets that are lower than STREAMER_PACKET_SIZE */
        if (soundSize - soundDataPlayed < STREAMER_PACKET_SIZE)
        {
            vTaskDelay(DELAY_BETWEEN_SOUND_PLAYS);
            soundDataPlayed = 0;
        }
        ...
        if (s_streamerPoolCnt > 0)
        {
            ...
            SLN_AMP_WriteNoWait(&s_streamerPools[s_streamerPoolIdx * STREAMER_PACKET_SIZE], packetSize);
...
void appTask(void *arg)
{
    ...
    SLN_AMP_SetVolume(STREAMER_VOLUME);
    ...
    if (xTaskCreate(soundPlayTask, sound_play_task_NAME, sound_play_task_STACK, NULL, sound_play_task_PRIORITY,
                    &xSoundPlayTaskHandle) != pdPASS)
    {
        PRINTF("xTaskCreate sound_play_task failed!\r\n");
    }

    while (1)
    {
        xTaskNotifyWait(0xffffffffU, 0xffffffffU, &taskNotification, pdMS_TO_TICKS(1000));
        ...
        switch (taskNotification)
        {
            case kWakeWordDetected:
            {
                ...
                RGB_LED_SetBrightnessColor(LED_BRIGHT_HIGH, LED_COLOR_BLUE);
                vTaskDelay(150);
                RGB_LED_SetColor(LED_COLOR_OFF);
...
void APPTask(void *handle)
{
    ...
    if (xTaskCreate(appTask, "APP_Task", 512, NULL, configMAX_PRIORITIES - 2, &xAppTaskHandle) != pdPASS)
    {
