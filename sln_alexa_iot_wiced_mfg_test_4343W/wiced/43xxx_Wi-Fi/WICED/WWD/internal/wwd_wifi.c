 ...
 */
static char* wiced_strnstr(const char *s, uint16_t s_len, const char *substr, uint16_t substr_len)
{
...
wwd_result_t wwd_wifi_get_ap_info( wl_bss_info_t* ap_info, wiced_security_t* security )
{
    ...
    *security = (wiced_security_t)(security_value & 0xFF);
...
wwd_result_t wwd_wifi_get_cap( char *buf, uint16_t buflen, char *cap )
{
    ...
    if ( cap && wiced_strnstr( buf, buflen, cap, (uint16_t)strlen( cap ) ) == NULL )
    {
